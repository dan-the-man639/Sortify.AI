...
export default function App() {
  ...
  const collectData = async (className) => {
    ...
    try {
      if (this.camera) {
        ...
        image = await resizeImage(photo.uri, 300, 300);
  ...
  return (
    <SafeAreaView style={(styles.container, { backgroundColor: "black" })}>
      ...
      <View style={{ width: 420, height: 600, marginTop: 30}}>
        {hasPermission && (
          <Camera
            style={{ flex: 1 }}
            type={type}
      ...
      <View style={styles.interaction}>
        <Text style={styles.thisgoesin}>This goes in...</Text>

        {prediction.label === "Class A" ? (
          <View style={styles.categoryContainer}>
            <Icon name="recycle" size={50} color="#065FB1" />
            <Text style={[{ color: "#065FB1" }, styles.categories]}>Recycling</Text>
          </View>
        ) : prediction.label === "Class B" ? (
          <View style={styles.categoryContainer}>
            <Icon name="trash" size={50} color="#898987" />
            <Text style={[{ color: "#898987", marginTop: 5 }, styles.categories]}>Garbage</Text>
          </View>
        ) : (
          <View style={styles.categoryContainer}>
            <IconMCI name="food-apple" size={50} color="#61D94C" />
            <Text style={[{ color: "#61D94C", marginTop: 5 }, styles.categories]}>Compost</Text>
          </View>
        )}
